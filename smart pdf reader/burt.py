...
def open_files():
    ...
    def links():
        ...
        def heading_links():
            # Abstract to introduction (page 1)
            pdf_writer.addLink(pagenum=0, pagedest=1, rect=RectangleObject([20, 745, 200, 765]), )
            # 2 to 2.1 (page 2)
            pdf_writer.addLink(pagenum=0, pagedest=2, rect=RectangleObject([20, 710, 400, 730]), )
            # 2.2 to 2.3 (page 3)
            pdf_writer.addLink(pagenum=0, pagedest=3, rect=RectangleObject([20, 690, 400, 700]), )
            # 2.4 (page 4)
            pdf_writer.addLink(pagenum=0, pagedest=4, rect=RectangleObject([20, 670, 400, 680]), )
            # 2.5 to 3.2 (page 6)
            pdf_writer.addLink(pagenum=0, pagedest=6, rect=RectangleObject([20, 620, 400, 660]), )
            # 3.3 to 3.4 (page 7)
            pdf_writer.addLink(pagenum=0, pagedest=7, rect=RectangleObject([20, 590, 400, 600]), )
            # 4 to 4.2 (page 8)
            pdf_writer.addLink(pagenum=0, pagedest=8, rect=RectangleObject([20, 550, 400, 575]), )
            # 4.3 to 4.4 (page 9)
            pdf_writer.addLink(pagenum=0, pagedest=9, rect=RectangleObject([20, 510, 400, 530]), )
            # 5 (page 10)
            pdf_writer.addLink(pagenum=0, pagedest=10, rect=RectangleObject([20, 500, 400, 507]), )
            # 6 to Conclusion (page 11)
            pdf_writer.addLink(pagenum=0, pagedest=11, rect=RectangleObject([20, 470, 400, 490]), )
            # References (page 12)
            pdf_writer.addLink(pagenum=0, pagedest=12, rect=RectangleObject([20, 450, 400, 460]), )

        def figure_links():
            # page 5; figure three on page 4
            pdf_writer.addLink(pagenum=5, pagedest=4, rect=RectangleObject([470, 194, 650, 205]), )
